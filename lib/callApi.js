    ...
    callApi: function (channelId, callback) {
        ...
        var processItems = function (i) {
            if (i < url.length) {
                ...
                // logger.info('get node info start');
                Thenjs(function(cont) {
                    totalNode = 0;
                    activeNode = 0;
                    activeTime = parseInt(Date.now()/1000);
                    cont();
                }).each(ucloud, function (cont, path, index) {
                    logger.debug('loop for ucloud ' + index);
                    Thenjs(function (cont2) {
                        ...
                        Thenjs.each(uHostSet, function (cont3, e, i) {
                            logger.debug('loop for uHostSet ' + i);
                            if (e.State == 'Running') {
                                activeNode++;
